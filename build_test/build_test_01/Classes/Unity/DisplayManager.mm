...
- (void)present
{
	...
	if(UnitySelectedRenderingAPI() == apiOpenGLES2 || UnitySelectedRenderingAPI() == apiOpenGLES3)
	{
		...
		GLES_CHK(glBindRenderbuffer(GL_RENDERBUFFER, self.surfaceGLES->systemColorRB));
		[UnityGetMainScreenContextGLES() presentRenderbuffer:GL_RENDERBUFFER];
	}
	...
	if(_needRecreateSurface)
	{
		RenderingSurfaceParams params =
		{
			...
			_surface->disableDepthAndStencil, false
		};

		if(UnitySelectedRenderingAPI() == apiOpenGLES2 || UnitySelectedRenderingAPI() == apiOpenGLES3)
			params.useCVTextureCache = self.surfaceGLES->cvTextureCache != 0;
...
- (void)presentAll
{
	for(UIScreen* screen in _displayConnection)
		[[_displayConnection objectForKey:screen] present];
}
...
- (void)presentAllButMain
{
	for(UIScreen* screen in _displayConnection)
	{
		...
		// in that case we dont want to present Display
		DisplayConnection* conn = [_displayConnection objectForKey:screen];
		if(screen != [UIScreen mainScreen] && conn.surface != nil)
			[[_displayConnection objectForKey:screen] present];
	}
}
...
static void EnsureDisplayIsInited(DisplayConnection* conn)
{
	...
	int api = UnitySelectedRenderingAPI();
	if(api == apiMetal)	needRecreate = conn.surfaceMTL->layer == nil;
