...
extern "C" void CreateUnityRenderBuffersGLES(UnityDisplaySurfaceGLES* surface)
{
	...
	int api = surface->context.API;
	{
		...
		if(surface->depthRB)
			...
			surface->unityDepthBuffer = UnityCreateDummySurface(api, surface->unityDepthBuffer, false, &target_desc);
	...
	if(surface->msaaFB || surface->targetFB)
	{
		...
		surface->systemDepthBuffer = UnityCreateDummySurface(api, surface->systemDepthBuffer, false, &system_desc);
...
extern "C" void PreparePresentGLES(UnityDisplaySurfaceGLES* surface)
{
	...
	if(surface->targetColorRT)
	{
		...
		UnitySetAsDefaultFBO(surface->context.API, surface->systemColorBuffer, surface->systemDepthBuffer);
		UnitySetFBO(surface->context.API, surface->systemColorBuffer, surface->systemDepthBuffer);
...
{
	UnitySetAsDefaultFBO(surface->context.API, surface->unityColorBuffer, surface->unityDepthBuffer);
}
