...
public class BuckinghamGooey : Potential {
	...
	public override void preCompute()
	{
		...
		for (int i = 0; i < Environment.myEnvironment.molecules.Length; i++)
		{
			Atom firstAtom = Environment.myEnvironment.molecules[i].GetComponent<Atom>();
			for (int j = 0; j < Environment.myEnvironment.molecules.Length; j++)
			{
				Atom secondAtom = Environment.myEnvironment.molecules[j].GetComponent<Atom>();
